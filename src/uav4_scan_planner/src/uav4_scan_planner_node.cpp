...
public:
        ...
        dis_(0.0, 1.0)
    {
        ...
        nh_.param<double>("region_start_x", region_start_x_, -10.0); //第一个区域的中心
        nh_.param<double>("region_start_y", region_start_y_, -9.0);
        ...
        nh_.param<double>("safe_distance", safe_distance_, 0.1);
        ...
        //初始化边界
        next_region_boundary_.x = region_start_x_;
        next_region_boundary_.y = region_start_y_;
...
private:
    ...
    double region_start_x_;
    double region_start_y_;
    ...
    void handleWaitState() {
        ...
        if (
            ...
            ROS_INFO("UAV4: Starting to collect mines for region %d", current_region_id_);
            current_state_ = UAV4State::COLLECTING;
        } else {
            ROS_INFO("UAV4: Waiting for UAV2 (%d) and UAV3 (%d) to complete region %d", 
                    uav2_status_.region_id, uav3_status_.region_id, current_region_id_);
        }
    }
    ...
    void handleCollectingState() {
        ...
        if (it != verified_mines_by_region_.end() && !it->second.empty()) {
            ROS_INFO("UAV4: Collected %zu mines for region %d. Starting path planning.", 
                    it->second.size(), current_region_id_);
            if(current_region_id_ == 0){ //初始化要飞到探索区域的边界
                moveToBoundary();
                current_position_ = next_region_boundary_;
            }
    ...
    void handlePlanningState() {
        ...
        if (generateSafePath(current_region_id_)) {
            ROS_INFO("UAV4: Generated safe path with %zu waypoints for region %d", 
                    current_path_.size(), current_region_id_);
            current_path_index_ = 0;
            current_state_ = UAV4State::MOVING;
        } else {
            ROS_ERROR("UAV4: Failed to generate safe path for region %d", current_region_id_);
            current_state_ = UAV4State::WAIT;
        }
    }
    ...
    void handleMovingState() { //移动到对面边界
        ...
        if (current_path_.empty()) {
            ...
            if (checkWaypointReached(next_region_boundary_)) {
                ROS_INFO("UAV4: Arrived at boundary of region %d", current_region_id_);
                finishRegion();
            ...
            if (current_path_index_ < current_path_.size()) {
                ...
                // 所有航点已发送完毕，检查是否到达终点
                if (checkWaypointReached(current_path_.back().pose.position) || waypoint_reached_) {
                    ROS_INFO("UAV4: 已完成区域%d的路径飞行", current_region_id_);
                    finishRegion();
                }
            }
        }
    ...
    void processMines(const mine_detection::MineArray::ConstPtr& msg, int uav_id) {
        // 前提：uav2/3必须的region_id是同步的
        int region_id = std::max(uav2_status_.region_id, uav3_status_.region_id) - 1;
    ...
    bool generateSafePath(int region_id) {
        ...
        // 使用A*规划路径
        astar_planner_->initRegionParm(region_width_,region_length_,region_start_x_,region_start_y_,region_id);
    ...
    geometry_msgs::Point calculateRegionBoundary(int region_id) {
        ...
        next_region_boundary.x = region_start_x_ + region_id * region_length_;
        next_region_boundary.y = region_start_y_;
        next_region_boundary.z = flight_height_;
        
        ROS_INFO("UAV4: Calculated boundary for region %d at (%.2f, %.2f, %.2f)", 
                region_id, next_region_boundary.x, next_region_boundary.y, next_region_boundary.z);
